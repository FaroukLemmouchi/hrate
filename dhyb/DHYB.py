...
def save_sample_data(acc_data, ibi_data):
    np.savetxt(os.path.join(BASE_DIR, "data", "sample_data_acc.csv"), np.column_stack((acc_data['times'], acc_data['values'])), delimiter=",")
    np.savetxt(os.path.join(BASE_DIR, "data", "sample_data_ibi.csv"), np.column_stack((ibi_data['times'], ibi_data['values'])), delimiter=",")

def load_sample_data():
    sample_acc_data = np.loadtxt(os.path.join(BASE_DIR, "data", "sample_data_acc.csv"), delimiter=",")
    acc_data = {'times': sample_acc_data[:, 0], 'values': sample_acc_data[:, 1:]}
    sample_ibi_data = np.loadtxt(os.path.join(BASE_DIR, "data", "sample_data_ibi.csv"), delimiter=",")
...
def entry_point(use_sample_data, record_len):
